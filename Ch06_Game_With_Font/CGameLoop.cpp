...
CGameLoop::CGameLoop() : 
	m_eGameMode(GameMode::GM_TITLE),
	m_isFirstGame(true),
	m_GameState(nullptr)
{
	CStringRenderer::Create("data/image/font.dds");
}
...
void CGameLoop::SelectStage()
{
	...
	if (m_eGameMode != m_eNextGameMode)
	{
		...
		m_eNextGameMode = GameMode::GM_NONE;
	}
...
void CGameLoop::InGame()
{
	...
	if (isCleared)
	{
		delete m_GameState;
		m_GameState = nullptr;
		m_eGameMode = GameMode::GM_CLEAR;
		m_eNextGameMode = GameMode::GM_MENU;
	}
